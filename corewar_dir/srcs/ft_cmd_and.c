...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/02/19 13:27:21 by kdezaphi          #+#    #+#             */
/*   Updated: 2015/03/10 10:14:09 by mgiraud-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static void	ft_cmd_and_verbose(t_vm *vm, int *jump, int *param,
	t_player *player)
{
	int	i;
...
static void	ft_cmd_and_run(t_vm *vm, t_player *player, char *coding, int *jump)
{
	int		param[3];
	int		i;
	...
	*jump = ft_get_param(vm, player, param, coding);
	...
	ft_cmd_and_verbose(vm, jump, param, player);
...
void		ft_cmd_and(t_vm *vm, t_player *player)
{
	...
	if ((coding[0] == 'r' || coding[0] == 'd' || coding[0] == 'i')
		...
		ft_cmd_and_run(vm, player, coding, &jump);
	else
