...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/02/22 22:12:24 by kdezaphi          #+#    #+#             */
/*   Updated: 2015/03/07 11:46:35 by mgiraud-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static void	ft_cmd_add_run(t_vm *vm, t_player *player, char *coding, int *jump)
{
	int		param[3];
	int		i;
	...
	*jump = ft_get_param(vm, player, param, coding);
	if (vm->verbose)
	{
		ft_printf("P %4d | add r%d r%d r%d\n",
			player->num_proc, param[0], param[1], param[2]);
		ft_printf("ADV %d (%#06x -> %#06x) ",
			*jump, player->pc, (player->pc + *jump) % MEM_SIZE);
		i = -1;
		while (++i < *jump)
...
void		ft_cmd_add(t_vm *vm, t_player *player)
{
	...
	if (coding[0] == 'r' && coding[1] == 'r' && coding[2] == 'r'
		&& coding[3] == 0)
		ft_cmd_add_run(vm, player, coding, &jump);
	else
