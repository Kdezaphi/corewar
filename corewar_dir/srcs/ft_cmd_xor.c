...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/02/22 22:35:24 by kdezaphi          #+#    #+#             */
/*   Updated: 2015/03/07 11:24:45 by mgiraud-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static void	ft_cmd_xor_verbose(t_vm *vm, t_player *player, int *jump,
	int *param)
{
	int	i;
...
static void	ft_cmd_xor_run(t_vm *vm, t_player *player, char *coding, int *jump)
{
	int		param[3];
	int		i;
	...
	*jump = ft_get_param(vm, player, param, coding);
	...
	if (vm->verbose)
		ft_cmd_xor_verbose(vm, player, jump, param);
...
void		ft_cmd_xor(t_vm *vm, t_player *player)
{
	...
	if ((coding[0] == 'r' || coding[0] == 'd' || coding[0] == 'i')
		...
		ft_cmd_xor_run(vm, player, coding, &jump);
	else
